...
namespace ml_kem {
// ML-KEM crypto key generation algorithm, generating byte serialized public key and secret key, given 32 -bytes seed `d`
// See algorithm 13 defined in ML-KEM specification https://doi.org/10.6028/NIST.FIPS.203.
    template<size_t k, size_t eta1>
    constexpr void
    crypto_keygen(std::span<const uint8_t, 32> d, // used in CPA-PKE
                  std::span<uint8_t, ml_kem_utils::get_pke_public_key_len(k)> pubkey,
                  std::span<uint8_t, ml_kem_utils::get_pke_secret_key_len(k)> seckey)requires(ml_kem_params::check_keygen_params(k, eta1)) {
        k_pke::keygen<k, eta1>(d, pubkey, seckey);
    }

// ML-KEM crypto algorithm, generating byte serialized public key and secret key, given 32 -bytes seed `d`
// See algorithm 14 defined in ML-KEM specification https://doi.org/10.6028/NIST.FIPS.203.
    ...
    crypto(std::span<const uint8_t, 32> m,
           std::span<const uint8_t, ml_kem_utils::get_pke_public_key_len(k)> pubkey,
           std::span<uint8_t, ml_kem_utils::get_pke_cipher_text_len(k, du, dv)> cipher)
           requires(ml_kem_params::check_encap_params(k, eta1, eta2, du, dv)) {
        const auto has_mod_check_passed = k_pke::encrypt<k, eta1, eta2, du, dv>(pubkey, m, m, cipher);
...
// ML-KEM decrypto algorithm, generating byte serialized public key and secret key, given 32 -bytes seed `d`
// See algorithm 15 defined in ML-KEM specification https://doi.org/10.6028/NIST.FIPS.203.
    template<size_t k, size_t eta1, size_t eta2, size_t du, size_t dv>
    constexpr void
    decrypto(std::span<const uint8_t, ml_kem_utils::get_pke_secret_key_len(k)> seckey,
             std::span<const uint8_t, ml_kem_utils::get_pke_cipher_text_len(k, du, dv)> cipher,
             std::span<uint8_t, 32> m)requires(ml_kem_params::check_decap_params(k, eta1, eta2, du, dv)) {
        k_pke::decrypt<k, du, dv>(seckey, cipher, m);
    }
...
// See algorithm 16 defined in ML-KEM specification https://doi.org/10.6028/NIST.FIPS.203.
    ...
    keygen(std::span<const uint8_t, 32> d, // used in CPA-PKE
           ...
           std::span<uint8_t, ml_kem_utils::get_kem_secret_key_len(k)> seckey)requires(ml_kem_params::check_keygen_params(k, eta1)) {
...
// See algorithm 17 defined in ML-KEM specification https://doi.org/10.6028/NIST.FIPS.203.
    ...
    encapsulate(std::span<const uint8_t, 32> m,
                ...
                std::span<uint8_t, 32> shared_secret)requires(ml_kem_params::check_encap_params(k, eta1, eta2, du, dv)) {
...
// See algorithm 18 defined in ML-KEM specification https://doi.org/10.6028/NIST.FIPS.203.
    ...
    decapsulate(std::span<const uint8_t, ml_kem_utils::get_kem_secret_key_len(k)> seckey,
                ...
                std::span<uint8_t, 32> shared_secret)requires(ml_kem_params::check_decap_params(k, eta1, eta2, du, dv)) {
