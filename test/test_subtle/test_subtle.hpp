...
// integer types, checking result against native comparison operator ( i.e. == )
    ...
    void test_ct_eq()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
...
// unsigned integer types, checking result against native comparison operator
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_ne()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
...
// over unsigned integer types.
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_select()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
...
// unsigned integer types.
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_swap()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
...
// over unsigned integer types, checking result against native comparison op.
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_le()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
...
// unsigned integer types, checking result against native comparison operator.
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_gt()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
...
// over unsigned integer types, checking result against native comparison op.
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_ge()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
...
// unsigned integer types, checking result against native comparison operation.
    template<typename operandT,
            typename returnT,
            const size_t iterations = (1ul << 16)>
    void test_ct_lt()requires(std::is_unsigned_v<operandT> && std::is_unsigned_v<returnT>) {
        constexpr returnT truthv = -static_cast<returnT>(1);
        constexpr returnT falsev = 0;
        std::random_device rd;
        std::mt19937_64 gen(rd());
